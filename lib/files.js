...
const getFilesForHostInPageModConfigSubDir = (pagemoddir, configdir, exts) => {
	const resource = pagemoddir.toLowerCase() + '-config';
	return hostname => {
		const extcopy = {};
		...
		forEach(filesByExt, (files, i) => {
			filesByExt[i] = files.map(name => {
				return [ resource, configdir, hostname, name ].reduce((result, value) => {
...
exports.runHostScript = (worker, configdir, hostname, scripts) => {
	const resource = 'resource://' + configdir.toLowerCase() + '-config/';
